...
def create_fca_warning_light(packer, CAN, frame):
  ...
  if frame % 2 == 0:
    values = {
      ...
      'SET_ME_FF': 0xff,
      'SET_ME_FC': 0xfc,
      'SET_ME_9': 0x1,  # CCNC: Carnival HEV ADAS ECU uses 0x1 (was 0x9 on prior HKG)
    }
...
def create_adrv_messages(packer, CAN, frame):
  ...
  if frame % 5 == 0:
    values = {
      'SET_ME_1C': 0x8,  # CCNC: Carnival HEV (was 0x1c on prior HKG)
      'SET_ME_FF': 0xff,
      'SET_ME_TMP_F': 0xf,
      'SET_ME_TMP_F_2': 0xf,
    }
    ...
    values = {
      'SET_ME_E1': 0x14,  # CCNC: Carnival HEV (was 0xe1 on prior HKG)
      'SET_ME_3A': 0x80,  # CCNC: Carnival HEV (was 0x3a on prior HKG)
    }
...
def create_ccnc(packer, CAN, openpilot_longitudinal_control, enabled, hud, left_blinker, right_blinker,
                msg_161, msg_162, msg_1b5, is_metric, out, main_cruise_enabled, lfa_icon):
  # Defensive: if the base CCNC messages were not captured (e.g. an LKA-steering CCNC
  # car), return early rather than emitting a message from an empty template.
  ...
  if openpilot_longitudinal_control:
    ...
    msg_162["LEAD_DISTANCE"] = msg_1b5["Longitudinal_Distance"] if msg_1b5 else 150
